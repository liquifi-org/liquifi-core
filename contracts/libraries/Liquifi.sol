...
library Liquifi {
    enum Flag {
        ...
        POOL_LOCKED,
        ARBITRAGEUR_FULL_FEE,
        GOVERNANCE_OVERRIDEN
    }
    ...
    struct PoolState {
        ...
        uint64 firstByTokenAStopLoss;
        uint64 lastByTokenAStopLoss; // linked list of orders sorted by (amountAIn/stopLossAmount) ascending
        uint64 firstByTokenBStopLoss;
        uint64 lastByTokenBStopLoss; // linked list of orders sorted by (amountBIn/stopLossAmount) ascending
        // saved on QUEUE_TIMEOUT_DIRTY in exit()
        uint64 firstByTimeout;
        uint64 lastByTimeout; // linked list of orders sorted by timeouts ascending
        // this field contains
    ...
    enum OrderFlag {NONE, IS_TOKEN_A, EXTRACT_ETH}

    struct Order {
        // optimized for storage, fits into 3 words
    ...
    struct OrderClaim {
        //in-memory only
        uint256 amountOut;
        uint256 orderFlowSpeed;
        uint256 orderId;
        uint256 flags;
        uint256 closeReason;
        uint256 previousAvailableBalance;
        uint256 previousFlowSpeed;
        uint256 previousOthers;
    }
    ...
    function _require(
        ...
        if (condition) return;
        {
            ...
            bytes memory message =
                "\x46\x41\x49\x4c\x20\x68\x74\x74\x70\x73\x3a\x2f\x2f\x65\x72\x72\x2e\x6c\x69\x71\x75\x69\x66\x69\x2e\x6f\x72\x67\x2f\x5f\x5f";
    ...
    function trimTime(uint256 time) internal pure returns (uint64 trimmedTime) {
        Liquifi._require(time <= maxTime, Liquifi.Error.C_TOO_BIG_TIME_VALUE, Liquifi.ErrorArg.A_NONE);
        return uint64(time);
    }

    function trimPeriod(uint256 period, Liquifi.ErrorArg periodType) internal pure returns (uint32 trimmedPeriod) {
        Liquifi._require(period <= ~uint32(0), Liquifi.Error.D_TOO_BIG_PERIOD_VALUE, periodType);
        return uint32(period);
    }

    function trimAmount(uint256 amount, Liquifi.ErrorArg amountType) internal pure returns (uint112 trimmedAmount) {
        Liquifi._require(amount <= ~uint112(0), Liquifi.Error.E_TOO_BIG_AMOUNT_VALUE, amountType);
        return uint112(amount);
    }

    function trimTotal(uint256 amount, Liquifi.ErrorArg amountType) internal pure returns (uint128 trimmedAmount) {
        Liquifi._require(amount <= ~uint128(0), Liquifi.Error.K_TOO_BIG_TOTAL_VALUE, amountType);
        return uint128(amount);
    }

    function trimFlowSpeed(uint256 amount, Liquifi.ErrorArg amountType) internal pure returns (uint144 trimmedAmount) {
    ...
    function checkFlag(PoolState memory _state, Flag flag) internal pure returns (bool) {
        return _state.packed & uint96(1 << uint256(flag)) != 0;
    }

    function setFlag(PoolState memory _state, Flag flag) internal pure {
        _state.packed = _state.packed | uint96(1 << uint256(flag));
    }
    ...
    function unpackGovernance(PoolState memory _state)
        internal
        pure
        returns (
            uint256 instantSwapFee,
            uint256 desiredOrdersFee,
            uint256 protocolFee,
            uint256 maxPeriod,
            uint256 desiredMaxHistory
        )
    {
    ...
    function setInvalidState(PoolState memory _state, Liquifi.ErrorArg reason) internal pure {
        ...
        uint256 oldReason = uint8(_state.packed);
        if (uint256(reason) > oldReason) {
    ...
    function isTokenAIn(uint256 orderFlags) internal pure returns (bool) {
        return orderFlags & uint256(Liquifi.OrderFlag.IS_TOKEN_A) != 0;
    }
}
