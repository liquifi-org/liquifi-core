...
contract TestLiquidityPool is LiquifiLiquidityPool {
    ...
    string public constant override symbol = "Liquifi Test Pool Token";

    constructor(
        address tokenAAddress,
        address tokenBAddress,
        address _governanceRouter
    ) LiquifiLiquidityPool(tokenAAddress, tokenBAddress, false, _governanceRouter) {
        savedState.notFee = 997; // fee = 0.3%
        savedState.packed = (/*instantSwapFee*/
        (uint96(3) << 88) |
            (uint96(3) << 80) | // 0.03% // 0.03%
            /*fee*/
            /*maxPeriod*/
            (uint96(1 hours) << 40) |
            /*desiredMaxHistory*/
    ...
    function actualizeBalances(Liquifi.ErrorArg)
        internal
        view
