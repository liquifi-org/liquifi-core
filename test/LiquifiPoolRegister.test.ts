...
describe("Liquifi Pool Register", () => {
    ...
    async function getFlowBreakEvent(fromBlock: number|undefined, pool: LiquifiDelayedExchangePool): Promise<LogDescription[]> {
        const eventFragment = pool.interface.getEvent("FlowBreakEvent");
        const topic = pool.interface.getEventTopic(eventFragment);
        const filter = { topics: [topic], address: pool.address, fromBlock };
        const swapLogs = await pool.provider.getLogs(filter);
        return swapLogs.map(log => pool.interface.parseLog(log));
	...
	it("should withdraw from an existing distribution pool", async () => {
		const provider = await liquidityProvider.getAddress()
		await addLiquidity(token(100000), token(10000));
		...
		expect(
			await register.connect(liquidityProvider).withdrawFromDistributionPool(
				tokenA.address, tokenB.address, token(100000))).to.be.ok;

		expect(await tokenA.balanceOf(provider)).to.eq(token(8900000))

        const pool = await LiquifiDelayedExchangePoolFactory.connect(await factory.findPool(tokenA.address, tokenB.address), factoryOwner);  
		const dpool = await register.distributionPools(pool.address);
		
		expect(dpool.balance).to.be.eq(token(900000))

	})

	it("cannot withdraw from a distribution pool if not the owner", async () => {
		await addLiquidity(token(100000), token(10000));
		...
		await expect(register.connect(otherTrader).withdrawFromDistributionPool(
				tokenA.address, tokenB.address, token(1000000))).to.be.revertedWith("LIQIFI: SENDER IS NOT DPOOL OWNER");
    })

	it("cannot withdraw from a distribution pool if wrong tokens order", async () => {
		await addLiquidity(token(100000), token(10000));
		...
		await expect(register.connect(liquidityProvider).withdrawFromDistributionPool(
				tokenB.address, tokenA.address, token(1000000))).to.be.revertedWith("LIQIFI: INVALID TOKEN ORDER IN DPOOL");
    })

	it("cannot withdraw from a distribution pool more than available", async () => {
		await addLiquidity(token(100000), token(10000));
		...
		await expect(register.connect(liquidityProvider).withdrawFromDistributionPool(
				tokenA.address, tokenB.address, token(1000001))).to.be.revertedWith("LIQUIFI: INSUFFICIENT DPOOL BALANCE");
    })
	...
	it("cannot update a distribution pool if wrong tokens order", async () => {
		await addLiquidity(token(100000), token(10000));
        ...
        await tokenB.connect(liquidityProvider).approve(register.address, token(1000000))
		await expect(register.connect(liquidityProvider).updateDistributionPool(
				tokenB.address, tokenA.address, token(1000000), BigNumber.from(100000), 255, token(100))).to.be.revertedWith("LIQIFI: INVALID TOKEN ORDER IN DPOOL");
    })
	...
	it("cannot remove an existing distribution pool if wrong tokens order", async () => {
		const provider = await liquidityProvider.getAddress()
		await addLiquidity(token(100000), token(10000));
		...
		await expect(register.connect(liquidityProvider).removeDistributionPool(
				tokenB.address, tokenA.address)).to.be.revertedWith("LIQIFI: INVALID TOKEN ORDER IN DPOOL");
	...
	it("should do delayed swap against a distribution pool", async () => {
		...
		const expectedCounterAmount = token(10000).mul(token(100000)).div(token(10000)).mul(200).div(256)
		const distributionPrice = (expectedCounterAmount.mul(997).div(1000).add(token(100000))).mul(1000000).div(token(10000).mul(997).div(1000).add(token(10000)))
		const minOutput = distributionPrice.mul(99).mul(token(10000)).div(100000000)
		...
		expect(delayedSwapEvents[0].args.minAmountOut).to.be.eq(minOutput);
		expect(delayedSwapEvents[1].args.minAmountOut).to.be.eq(counterAmount.mul(50000).div(1000000));

        await wait(ethers, 400);

		await pool.connect(otherTrader).processDelayedOrders();
		...
		expect(counterAmount).to.be.eq(expectedCounterAmount)		
		
		expect(await tokenA.balanceOf(trader)).to.be.gt(token(10000000).add(minOutput))
		expect(await tokenA.balanceOf(trader)).to.be.lte(token(10000000).add(token(10000).mul(distributionPrice).div(1000000)))

		expect(await tokenB.balanceOf(provider)).to.be.gt(token(9990000).add(counterAmount.mul(1000000).div(distributionPrice).mul(99).div(100)))
		expect(await tokenB.balanceOf(provider)).to.be.lt(token(9990000).add(counterAmount.mul(1000000).div(distributionPrice).mul(101).div(100)))
	...
	it("should not do a counter swap when a below the minimum amount", async () => {
		const provider = await liquidityProvider.getAddress()
		const trader = await otherTrader.getAddress()
		await addLiquidity(token(100000), token(10000))
        ...
        const time0 = await lastBlockTimestamp(ethers);
        await tokenB.connect(otherTrader).approve(register.address, token(99));
        const tx = await register.connect(otherTrader).delayedSwap(
            tokenB.address, 
            token(99), 
            tokenA.address, 
	...
	it("should do 2 parallel delayed swaps without stop loss", async () => {
		...
		const trader = await otherTrader.getAddress()
		//await addLiquidity(token(100000), token(10000))
        await token1.connect(liquidityProvider).approve(register.address, token(100000))
        await token2.connect(liquidityProvider).approve(register.address, token(10000));
        await register.connect(liquidityProvider).deposit(token1.address, token(100000), token2.address, token(10000), 
            await liquidityProvider.getAddress(), 42949672960);

		const inAmount = token(10000)
		const outAmount = token(78125)
		const distributionPrice = (outAmount.mul(997).div(1000).add(token(100000))).mul(1000000).div(inAmount.mul(997).div(1000).add(token(10000)))
		
        const time0 = await lastBlockTimestamp(ethers);
        await token2.connect(otherTrader).approve(register.address, token(10000));
        const tx = await register.connect(otherTrader).delayedSwap(
            token2.address, 
            inAmount, 
            token1.address, 
		...
		await token1.connect(liquidityProvider).approve(register.address, token(78125));
        await register.connect(liquidityProvider).delayedSwap(
            token1.address, 
            outAmount, 
            token2.address, 
            1,
        ...
        const pool = await LiquifiDelayedExchangePoolFactory.connect(await factory.findPool(token1.address, token2.address), factoryOwner);  
        const delayedSwapEvents = await getDelayedSwapEvent(tx.blockNumber);
       
        expect(delayedSwapEvents[0].args.tokenIn).to.eq(token2.address);
		const directOrderId = delayedSwapEvents[0].args.orderId;
        expect(delayedSwapEvents[1].args.tokenIn).to.eq(token1.address);
		const counterOrderId = delayedSwapEvents[1].args.orderId;
		const counterAmount = delayedSwapEvents[1].args.amountIn;
        ...
        const [orderHash1, orderHistoryList1] = await orderHistory(pool, tx, directOrderId);
        expect(await register.connect(otherTrader).claimOrder(token1.address, token2.address, orderHash1, orderHistoryList1,  time0.add(20000))).to.be.ok;

        const [orderHash2, orderHistoryList2] = await orderHistory(pool, tx, counterOrderId);
        expect(await register.connect(liquidityProvider).claimOrder(token1.address, token2.address, orderHash2, orderHistoryList2,  time0.add(20000))).to.be.ok;

		expect(await token1.balanceOf(provider)).to.be.eq(token(9900000).sub(outAmount))
		
		expect(await token1.balanceOf(trader)).to.be.gt(token(10000000).add(token(10000).mul(distributionPrice).div(1000000).mul(99).div(100)))
		expect(await token1.balanceOf(trader)).to.be.lte(token(10000000).add(token(10000).mul(distributionPrice).div(1000000).mul(101).div(100)))

		expect(await token2.balanceOf(provider)).to.be.gt(token(9990000).add(outAmount.mul(1000000).div(distributionPrice).mul(99).div(100)))
		expect(await token2.balanceOf(provider)).to.be.lt(token(9990000).add(outAmount.mul(1000000).div(distributionPrice).mul(101).div(100)))
